...
#Salmon RNA Alignment Endoderm
cd /media/storageA/hani/Apo_LUTI_Project/RNA_Endoderm_1_2

array=()
while IFS=  read -r -d $'\0'; do
    array+=("$REPLY")
done < <(find /media/storageA/hani/Apo_LUTI_Project/RNA_Endoderm_1_2/raw_fastq -type l -regex ".*R1.fastq.gz" -print0)

for i in ${array[@]}; do 
    echo Found File: $i
    my_path=$(echo $i | grep -Po ".*(?=/raw_fastq)")
    read_prefix=$(echo $i | grep -Po ".*(?=R1.fastq.gz)")
    r2_path=$read_prefix"R2.fastq.gz"
    echo Read 2 File: $r2_path
    my_name=$(echo $i | grep -Po "(?<=raw_fastq/JK12_)BC[0-9]+(?=_R1.fastq.gz)")
    echo Writing to $my_path"/SalmonOut_RNA_"$my_name
    salmon quant --no-version-check --libType A --seqBias --gcBias --index /media/storageA/hani/Apo_LUTI_Project/hg38_annotations/hg38_genocode_v32_SalmonIndex -1 $i -2 $r2_path -o $my_path"/SalmonOut_RNA_"$my_name -p 18 --validateMappings
done
